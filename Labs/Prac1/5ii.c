...
static void integral_image(const unsigned char * in, unsigned int * out) {
    ...
    for (unsigned int y = 1; y < HEIGHT; ++y) {
        row_sum = 0;
        for (unsigned int x = 0; x < WIDTH; x += BWSIZE) {
            for(unsigned w = x; w < x + BWSIZE; w += 4){
            //for(unsigned z = 0; z < BWSIZE; z += 4){
...
static void integral_image2(const unsigned char * in, unsigned int * out) {
    unsigned int row_sum = 0;
    ...
    for (unsigned int y = 1; y < HEIGHT; ++y) {
        row_sum = 0;
        for (unsigned int x = 0; x < WIDTH; x += BWSIZE) {
            for(unsigned w = x; w < x + BWSIZE; ++w){
                //for(unsigned z = x; z < x + BWSIZE; ++z){
                    row_sum += in[idx(w, y, WIDTH)];
                    out[idx(w, y, WIDTH)] = row_sum + out[idx(w, y-1, WIDTH)];
                //}
            }
...
static void integral_image3(const unsigned char * in, unsigned int * out) {
    unsigned int row_sum = 0;
